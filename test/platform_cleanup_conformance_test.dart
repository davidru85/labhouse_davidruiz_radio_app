...
void main() {
  group('Platform Cleanup & Naming Conformance', () {
    ...
    group('App identifiers (ADR-0003)', () {
      test('Android applicationId and namespace are com.labhouse.davidruizassessment.radioapp', () {
        final gradle = File('android/app/build.gradle.kts').readAsStringSync();
        expect(
          gradle.contains('applicationId = "com.labhouse.davidruizassessment.radioapp"'),
          isTrue,
          reason: 'Android applicationId must be com.labhouse.davidruizassessment.radioapp (ADR-0003)',
        );
        expect(
          gradle.contains('namespace = "com.labhouse.davidruizassessment.radioapp"'),
          isTrue,
          reason: 'Android namespace must be com.labhouse.davidruizassessment.radioapp (ADR-0003)',
        );
      });

      test('iOS bundle identifier is com.labhouse.davidruizassessment.radioapp', () {
        final pbxproj = File('ios/Runner.xcodeproj/project.pbxproj').readAsStringSync();
        expect(
          pbxproj.contains('PRODUCT_BUNDLE_IDENTIFIER = com.labhouse.davidruizassessment.radioapp;'),
          isTrue,
          reason: 'iOS PRODUCT_BUNDLE_IDENTIFIER must be com.labhouse.davidruizassessment.radioapp (ADR-0003)',
        );
        expect(
          pbxproj.contains('com.example'),
          isFalse,
          reason: 'iOS project must not retain any com.example bundle identifier',
        );
      });
      ...
      test('display name is RadioApp on Android and iOS', () {
        ...
        expect(
          infoPlist.contains('<key>CFBundleDisplayName</key>\n\t<string>RadioApp</string>'),
          isTrue,
      ...
      test('the conforming MainActivity exists under the new package path', () {
        ...
        expect(File(newPath).existsSync(), isTrue, reason: 'MainActivity must live under the new package path');
        expect(
          File(newPath).readAsStringSync().contains('package com.labhouse.davidruizassessment.radioapp'),
          isTrue,
          reason: 'MainActivity package declaration must match the new applicationId',
        );
        expect(
    ...
    group('Minimum OS versions (ADR-0002)', () {
      test('Android SDK levels are minSdk=23, targetSdk=34, compileSdk=34', () {
        final gradle = File('android/app/build.gradle.kts').readAsStringSync();
        expect(gradle.contains('minSdk = 23'), isTrue, reason: 'Android minSdkVersion must be 23 (ADR-0002)');
        expect(gradle.contains('targetSdk = 34'), isTrue, reason: 'Android targetSdkVersion must be 34 (ADR-0002)');
        expect(gradle.contains('compileSdk = 34'), isTrue, reason: 'Android compileSdkVersion must be 34 (ADR-0002)');
      });
