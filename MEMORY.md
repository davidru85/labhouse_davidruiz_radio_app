...
* **Current Task:** Phase 6, Sub-task 6.5 (`HistoryRepositoryImpl`) — **REFACTOR/PR-ready (PR open)** on branch `feature/phase-6-sub-task-65-history-repository-impl` (branched from synced `main` @ `e54a6de` after PR #30 merged, per ADR-0033/0035). Backed by `LocalHistoryDataSource` and enforcing the 50-item cap (`maxItems`) with FIFO eviction and move-to-top on replay (per ADR-0021): `addToHistory` reads the current list, and for an existing `stationUuid` does remove+re-add (Hive `put` updates a key in place, so the explicit remove promotes the entry to newest), otherwise evicts `current.first` when length ≥ 50, then appends. A broadcast `historyStream` re-reads and emits after each mutation; a `_guard<T>` maps local-storage errors to `StorageReadWriteFailure`. RED (7 failing tests) at `c3c4122`; GREEN (7 passing, `flutter analyze` clean) at `2b8745c`, pushed. REFACTOR: no production changes (code already minimal); docs only. Governed by ADR-0021, ADR-0037.
* **Last Completed Task:** Phase 6, Sub-task 6.3 (`GenresRepositoryImpl`), merged via PR #30 (`e54a6de`, now the `main` tip). Network-first over `RemoteGenresDataSource`, returns the fetched list truncated to `limit` (default 50; `_limited` preserves order). On remote `NetworkException`: falls back to `getCachedGenres()` when non-empty, else surfaces `exception.failure`. Caching best-effort via `_cacheBestEffort`.
* **Active Branch:** `feature/phase-6-sub-task-65-history-repository-impl` (branched from synced `main` after PR #30 merged, per ADR-0035).
* **Next Up:** remainder of **Phase 6 — Repository Implementations** (6.4 `CountriesRepositoryImpl` and 6.6 `PlaybackUrlRepositoryImpl` are already in flight on their own branches with open PRs), then `AudioPlayerRepositoryImpl` (own sub-task; design `just_audio`/`audio_service` port first), `ConnectivityRepositoryImpl` (ADR-0013), `NoOpAnalyticsRepositoryImpl` (ADR-0019). Wire local data sources per ARCHITECTURE.md §"Repository Contracts"/§"Dependency Rule".
