...
* **Current Task:** Phase 7, **Step 1 = combined sub-tasks 7.1–7.4** (`RadioPlayerBloc`, `StationsBloc`, `FavoritesBloc`, `HistoryBloc`) — **GREEN (4 BLoCs implemented; full suite green, analyze clean), uncommitted, in review** on branch `feature/phase-7-sub-task-71-74-core-blocs` (branched from synced `main` @ `1071f61` after PR #34 merged, per ADR-0033/0035; bundling 7.1–7.4 into one step is a user-approved one-off exception, mirroring the 6.7 bundling). BLoC tests are use-case-only (ARCHITECTURE.md §"Dependency Rule"): since use cases are `final` (unmockable), tests **mock repository contracts and build real use cases**. Phase-3 backfill added (TDD, GREEN): `GetFavoritesUseCase`, `WatchPlayerStateUseCase`, `WatchNowPlayingUseCase`. RadioPlayer is stream-driven and a play request **optimistically emits Buffering** before Playing/Error (ADR-0015/0025); BLoC states are named `RadioPlayer*` to avoid clashing with the domain `PlayerState` subclasses. **Analytics instrumentation (ROADMAP task 8) is deferred to Step 2** (sub-tasks 7.5–7.8: Genres/Countries/Connectivity BLoCs + analytics). Nothing committed yet — running `/review_my_code` before commit. Recurring review false-positive: reviewers asked for mocktail **closure** style, but `very_good_analysis` `unnecessary_lambdas` requires **tear-offs** for no-arg stubs (`when(repo.method)`), so keep tear-offs.
...
* **Next Up:** finish Step 1 (implement the 4 BLoCs GREEN, then REFACTOR/docs, push). Then **Step 2 = sub-tasks 7.5–7.8** (`GenresBloc`, `CountriesBloc`, `ConnectivityBloc`, analytics instrumentation via `TrackAnalyticsEventUseCase` per ADR-0019). ROADMAP Phase 7 items stay unchecked until each is REFACTOR-complete.
