...
* **Last Completed Task:** Phase 6, Sub-task 6.5 (`HistoryRepositoryImpl`), merged via PR #32 (`5cdea44`, now the `main` tip). Backed by `LocalHistoryDataSource`, enforcing the 50-item cap (`maxItems`) with FIFO eviction and move-to-top on replay via remove+re-add (per ADR-0021); broadcast `historyStream` re-reads and emits after each mutation; `_guard<T>` maps storage errors to `StorageReadWriteFailure`. (6.4 `CountriesRepositoryImpl` merged earlier via PR #31 `8dba3b6`.)
...
* **Next Up:** after 6.6 merges, the remaining **Phase 6 — Repository Implementations** are `AudioPlayerRepositoryImpl` (own sub-task; design `just_audio`/`audio_service` port first), `ConnectivityRepositoryImpl` (ADR-0013), `NoOpAnalyticsRepositoryImpl` (ADR-0019). Wire local data sources per ARCHITECTURE.md §"Repository Contracts"/§"Dependency Rule".
