...
* **Current Task:** **None — v1 is feature-complete + visually polished.** `main` @ `e67f89a` (UI Visual Polish merged via **PR #48**). What remains before release is non-code: the `VALIDATION_CHECKLIST.md` pass and the manual on-device a11y walkthrough.
* **Last Completed Task:** **UI Visual Polish — all 4 slices merged via PR #48** (squash, `main` @ `e67f89a`). One branch (`feature/ui-visual-polish`), each slice RED→GREEN→REFACTOR, 488 tests green, analyze clean, **verified live on the Android emulator + iOS simulator**. Slices: **(1) Portrait-only lock** (ADR-0004): `lockPortraitOrientation()` via `SystemChrome` + manifest/`Info.plist` portrait-only. **(2) Dark theme + Inter + glassmorphism** + **new ADR-0042** (evaluated Apple **Liquid Glass** and **DEFERRED** it — Flutter 3.41 ships no native Liquid Glass, it would need iOS 26+ vs the ADR-0002 floor and/or a dep vs ADR-0018 — so the iOS build stays Cupertino with the shared glass treatment). Added `lib/presentation/theme/` (`app_colors.dart` DESIGN tokens, `app_theme.dart` Material+Cupertino dark; Inter bundled in `assets/fonts/` per TECHNICAL_SPEC §10) + `GlassSurface` (BackdropFilter blur 20/40). **(3) Station artwork** = `StationArtwork` (`cached_network_image` + platform-adaptive icon fallback) on rows (64dp), favorites cards, mini-player (48dp), full-player hero. **(4) Full Player polish** = `LiveNowIndicator` (pulsing playback-accent dot; **infinite anim → tests use `pump()`/`pump(Duration)` not `pumpAndSettle`**), Live/Buffering status line (ARB `playerLiveStatus`/`playerBufferingStatus`), primary-colored mini-player progress bar on Favorites, glass app bars (`glassAppBar`), favorite glass chips, full-player atmospheric gradient + `GlassSurface.panel`. Reusable widgets added: `GlassSurface`(+`.panel`), `StationArtwork`, `LiveNowIndicator`, `glassAppBar`.
* **Active Branch:** `main` (synced @ `e67f89a`). The `feature/ui-visual-polish` branch is merged; a docs-sync branch follows with this tracker/ARCHITECTURE/TODO update.
* **Next Up:** **No roadmap code remains.** Release-gate, non-code items: the `VALIDATION_CHECKLIST.md` pass + the **manual on-device a11y walkthrough** (`docs/accessibility-checklist-phase9.md`). Known pre-existing functional gap (not introduced by the polish, no ticket yet): the Stations list starts empty until a search — `buildShellScope` creates `StationsBloc` without dispatching an initial popular-stations load. Deferred post-v1 features live in `TODO.md`.
