...
* **Current Task:** Phase 9 **Step 3 — tasks 9.3–9.6** on branch `feature/phase-9-sub-task-93-96-indexedstack-shell-miniplayer` (off `main` @ `c46710d`, which already includes the #43 Android-boot fix). **Task 9.3 DONE (RED→GREEN→REFACTOR), pushed, not yet PR'd.** Slices: (a) router migrated `ShellRoute`→`StatefulShellRoute.indexedStack` (both branches `preload: true`) — RED `f21fb13`, GREEN `6790afb`; (b) `MyApp` wired to `MaterialApp.router` + root `RadioPlayerBloc` (composition-root helpers `resolveRootPlayerBloc`/`buildShellScope` keep `GetIt` confined to `core/di`; `widget_test.dart` updated off the removed placeholder) — RED `98804ca`, GREEN `6e44e63`; REFACTOR = ADR-0041 + this tracker + ROADMAP checkoff (no code change). 394 tests green, analyze clean. **NEXT in this branch: task 9.4** (explicit scroll-offset preservation test across tabs), RED first.
...
* **Active Branch:** `feature/phase-9-sub-task-93-96-indexedstack-shell-miniplayer` (Step 3; task 9.3 done + pushed, 9.4–9.6 pending on the same branch).
* **Next Up:** **Task 9.4** (preserve scroll state across tabs) on the same branch, RED first — then 9.5 `MiniPlayerWidget` and 9.6 visibility animation. One PR for the whole Step 3 once 9.6 lands; the user owns the merge.
