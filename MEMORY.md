...
* **Current Task:** Phase 7, **Step 1 = combined sub-tasks 7.1–7.4** (`RadioPlayerBloc`, `StationsBloc`, `FavoritesBloc`, `HistoryBloc`) — **REFACTOR/PR-ready** on branch `feature/phase-7-sub-task-71-74-core-blocs` (from synced `main` @ `1071f61` after PR #34, per ADR-0033/0035; bundling 7.1–7.4 is a user-approved one-off exception). Commits: use-case RED `c6dff4c` → GREEN `6d5a35b` → BLoC RED `ec6c4de` → BLoC GREEN `8c94efa` (**pushed**, ADR-0036). BLoC tests are use-case-only (ARCHITECTURE.md §"Dependency Rule"): use cases are `final` (unmockable), so tests **mock repository contracts and build real use cases**. Phase-3 backfill: `GetFavoritesUseCase`, `WatchPlayerStateUseCase`, `WatchNowPlayingUseCase`. RadioPlayer is stream-driven; a play request **optimistically emits Buffering** before Playing/Error (ADR-0015/0025); states named `RadioPlayer*` (avoid clash with domain `PlayerState`). StationsBloc: trim before search, dispose-cancellation in `close()`, `STATIONS_MAX_LIMIT` cap (`maxStations`, default 100) — all per ADR-0014/0031. Full suite **322 green**, analyze clean; reviewed via `/review_my_code` (Approved). Open polish (non-blocking, deferred): N1 popular-predicate dispersion, N2 pause/stop comment, N3 history-add wiring note. **Analytics instrumentation (ROADMAP task 8) deferred to Step 2.** Recurring review false-positive: reviewers ask for mocktail **closure** style, but `very_good_analysis`/`unnecessary_lambdas` requires **tear-offs** for no-arg stubs — keep tear-offs.
* **Last Completed Task:** Phase 6 (Repository Implementations) — fully merged; 6.7 (Audio+Connectivity+NoOpAnalytics) via PR #34 (`1071f61`, now the `main` tip); 6.4/6.5/6.6 via PRs #31/#32/#33. Concrete `just_audio`/`audio_service` adapter + ADR-0022 controls remain deferred to composition-root wiring (integration test).
* **Active Branch:** `feature/phase-7-sub-task-71-74-core-blocs` (branched from synced `main` @ `1071f61` after PR #34 merged, per ADR-0035).
* **Next Up:** Step 1 PR is the user's to open/merge (per ADR-0035, branch the next step from synced `main` only after 7.1–7.4 merges). Then **Step 2 = sub-tasks 7.5–7.8** (`GenresBloc`, `CountriesBloc`, `ConnectivityBloc`, analytics instrumentation via `TrackAnalyticsEventUseCase` per ADR-0019).
