...
* **Current Task:** Phase 9 **Step 4 — tasks 9.7–9.10 ALL DONE** on branch `feature/phase-9-sub-task-97-910-fullplayer-nav-tabs-offline-banner` (branched off synced `main` @ `f6c5f90`, i.e. AFTER the Step 3 PR #44 merge). RED→GREEN→REFACTOR complete: **RED `ad10d38`** (failing tests + shared `test/support/shell_chrome_harness.dart`; reviewed/approved), **GREEN `92f5032`** (shell chrome, pushed), **REFACTOR** = made `AppShell.child` optional + dropped the redundant `child` arg from the router (chrome path uses `navigationShell`), ROADMAP 9.7–9.10 checkoff + this tracker (no behaviour change). **Pushed; PR to open for the user to merge.** Slices: **9.7** `MiniPlayerWidget` mounted in `AppShell` chrome, `onTap → context.go('/player')`, with a `Semantics` "Open player" button label + a `kMinInteractiveDimension` (48dp) touch target (ADR-0006 / TECHNICAL_SPEC §10). **9.8** `/player` is a `CustomTransitionPage` with a bottom-to-top `SlideTransition` (`Offset(0,1)→0`); stays a top-level route outside the shell (ADR-0041). **9.9** adaptive tab bar via `PlatformBuilder` (`BottomNavigationBar` Android / `CupertinoTabBar` iOS) driving `navigationShell.goBranch`. **9.10** global `OfflineBanner` ([lib/presentation/widgets/offline_banner.dart](lib/presentation/widgets/offline_banner.dart)) `BlocConsumer<ConnectivityBloc>`: "You're offline" while offline, transient "Back online" 2s on reconnection, nothing steadily online (ADR-0013). **SCOPE DECISION (supersedes the earlier "root-scoped" plan note below): `ConnectivityBloc` is provided in the shell scope (`buildShellScope`), not root** — the banner lives inside the shell, and the out-of-shell `/player` route does not currently need connectivity, so shell-scope (disposed with the shell) is sufficient and matches ADR-0013's "app-shell level". **411 tests green, analyze clean.** Reviewed via `/review_my_code` (RED+GREEN Approved) and several `/fix_review_issues` passes (added the mini-player Semantics label + 48dp touch target; rebutted false positives: "needs Cupertino MiniPlayer", "missing RED evidence" [RED is `ad10d38`], premature MEMORY/ROADMAP checkoff during GREEN).
* **Last Completed Task:** Phase 9 **Step 3 — tasks 9.3–9.6 (IndexedStack shell, scroll preservation, MiniPlayer + visibility)** — **merged via PR #44 (`main` now @ `f6c5f90`)**. `StatefulShellRoute.indexedStack` shell + `MyApp`→`MaterialApp.router` wiring (ADR-0041), scroll-preservation guard, and `MiniPlayerWidget` (buffering distinct from playing per ADR-0015, `AnimatedSize` collapse on idle). RED→GREEN per slice (`f21fb13`/`6790afb`, `98804ca`/`6e44e63`, `fb6e3c2`, `b70f1b1`/`80e9595`) + REFACTOR docs `f88736e`.
* **Active Branch:** `feature/phase-9-sub-task-97-910-fullplayer-nav-tabs-offline-banner` (Step 4; RED→GREEN→REFACTOR done + pushed, PR to open for the user to merge).
* **Next Up:** Open the Step 4 PR; the user merges + syncs `main`. Then **Step 5 (tasks 9.11–9.13)**: per-screen offline copy + retry (ADR-0013), render `nowPlaying` in Mini/Full player with station-name fallback (ADR-0012), and the visual accessibility checklist (ADR-0006: contrast, TalkBack/VoiceOver, focus). NB the **play-on-tap wiring** (dispatching `RadioPlayerPlayRequested` when a station row is tapped) is still absent — without it the mini-player never appears on device; fold it into Step 5 or a dedicated slice.
