...
* **Current Task:** Phase 9 **Step 3 — tasks 9.3–9.6 ALL DONE** on branch `feature/phase-9-sub-task-93-96-indexedstack-shell-miniplayer` (off `main` @ `c46710d`, which already includes the #43 Android-boot fix), **pushed, PR open for the user to merge.** Slices: **9.3** (a) router migrated `ShellRoute`→`StatefulShellRoute.indexedStack` (both branches `preload: true`) — RED `f21fb13`, GREEN `6790afb`; (b) `MyApp` wired to `MaterialApp.router` + root `RadioPlayerBloc` (composition-root helpers `resolveRootPlayerBloc`/`buildShellScope` keep `GetIt` confined to `core/di`) — RED `98804ca`, GREEN `6e44e63`; ADR-0041 `d1adacd`. **9.4** scroll preservation guard test — RED `fb6e3c2` (effectively GREEN via the IndexedStack keep-alive from 9.3). **9.5 + 9.6** `MiniPlayerWidget` + visibility animation ([lib/presentation/widgets/mini_player.dart](lib/presentation/widgets/mini_player.dart)) — RED `b70f1b1`, GREEN `80e9595`: `BlocBuilder<RadioPlayerBloc>` renders the station name for buffering/playing/paused, shows `AdaptiveProgressIndicator` ONLY while buffering (ADR-0015), wraps content in an always-mounted `AnimatedSize` collapsing to height 0 on idle. REFACTOR = no-op (code already minimal) + this tracker + ROADMAP checkoff. **400 tests green, analyze clean.** Reviewed via `/review_my_code` (Approved) + `/fix_review_issues` (rebutted a false-positive "needs Cupertino MiniPlayer" blocker — mini-player is shell chrome not a screen). **NEXT: open/merge the Step 3 PR, sync `main`, then Step 4 (9.7–9.10).**
...
* **Active Branch:** `feature/phase-9-sub-task-93-96-indexedstack-shell-miniplayer` (Step 3; tasks 9.3–9.6 all done + pushed, PR open for the user to merge).
* **Next Up:** Merge the Step 3 PR, sync `main`, then start **Step 4 (tasks 9.7–9.10)** — navigate to `FullPlayerScreen` on mini-player tap, bottom-to-top slide transition, tab bars (`BottomNavigationBar`/`CupertinoTabBar`), and the global offline banner (ADR-0013). Branch off freshly-synced `main`, RED first.
