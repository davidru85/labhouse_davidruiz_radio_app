...
* **Active Branch:** `feature/phase-9-sub-task-97-910-fullplayer-nav-tabs-offline-banner` (Step 4; RED `ad10d38` committed, GREEN in the working tree pending the user's commit&push).
* **Next Up:** Commit & push the Step 4 GREEN, then the REFACTOR docs pass (check off ROADMAP 9.7–9.10, finalize this tracker), open the Step 4 PR for the user to merge. After merge + sync, **Step 5 (tasks 9.11–9.13)**: per-screen offline copy + retry (ADR-0013), render `nowPlaying` in Mini/Full player with station-name fallback (ADR-0012), and the visual accessibility checklist (ADR-0006).
